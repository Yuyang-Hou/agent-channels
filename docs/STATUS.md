...
产品级 Skill、可编辑 Markdown 收发消息模板、重放幂等、按需会话搜索及浮层层级修复、成员身份恢复、客户端日志导出与固定内测签名已打入本地 `0.3.0-beta.15` Apple Silicon 验收包。发送成功模板仍待真实会话验收。
...
| Markdown 发送成功卡片 | 本地 Beta 包已完成，待实机 | 每 Subscription 可编辑；仅可靠回执展示，不改写频道正文或制造自回声 turn |
