...
private struct PijooV2SelfTest {
    static func main() throws {
        ...
        let webInvitation = try InvitationCodec.webURL(invitation)
        precondition(webInvitation == "https://example.test/join/quiet-owl-0001#invite=invite-secret")
        let decodedWebInvitation = try InvitationCodec.decode(webInvitation)
        precondition(decodedWebInvitation == invitation)
        ...
        precondition(requiresCodexRestart(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: nil))
        precondition(requiresCodexRestart(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: "mcp-old"))
        precondition(!requiresCodexRestart(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: "mcp-new"))
        precondition(!requiresCodexRestart(configured: false, expectedMCPVersion: "mcp-new", loadedMCPVersion: nil))
        precondition(generatedLocalNickname(id: UUID(uuidString: "12345678-0000-0000-0000-000000000000")!) == "Pijoo用户-123456")
        precondition(codexIntegrationReadiness(configured: false, expectedMCPVersion: "mcp-new", loadedMCPVersion: nil) == .notConfigured)
        precondition(codexIntegrationReadiness(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: nil) == .awaitingRestart)
        precondition(codexIntegrationReadiness(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: "mcp-old") == .versionMismatch)
        precondition(codexIntegrationReadiness(configured: true, expectedMCPVersion: "mcp-new", loadedMCPVersion: "mcp-new") == .ready)
