...
- [x] 在设置页两个 Codex 状态刷新入口和“转发到会话”的重新检查入口显示 loading 与重复点击保护
...
- [x] 区分用户主动暂停与 Host 会话未连接，首次断连时后台自动请求连接，恢复后自动续接；频道和会话入口显示红点，每张会话卡以统一图标提供打开、带进度反馈的刷新和展开操作，主窗口提示背景不延伸到标题栏
