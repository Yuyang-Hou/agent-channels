...
- 首次经用户确认安装一个固定 STDIO MCP 回复工具；后续换频道只更新本机 Binding，不再修改
  MCP 配置。ChatGPT 按官方流程在首次保存 MCP 后重启一次。
- 入站消息携带一次性 `reply_ref`；AI 只能用它向原发送者回复文本，不能选择 token、session、
  Host Binding 或任意广播目标。
- App 明确展示频道、ChatGPT、task、回复工具和最近投递状态，只在需要人工处理时通知。
...
- Intel Mac、Windows、自动更新、Developer ID 公证与公开分发。
